...
static const uint INPUT_W = 640;
static const uint INPUT_H = 640;
...
cv::Mat static_resize(cv::Mat &img) {
    ...
    std::cout << unpad_h << " " << unpad_w << std::endl;
...
static void draw_objects(const cv::Mat &bgr, const std::vector<Object> &objects, std::string f) {
    static const char *class_names[] = {
            "gate", "tommy_gun", "badge", "bootlegger", "g-man", "bottle", "barrel", "phone", "paper"
    };
...
int main(int argc, char **argv) {
    ...
    if (argc == 4 && std::string(argv[2]) == "-i") {
        ...
        std::cerr << "./yolox ../model_trt.engine -i ../../../assets/dog.jpg  // deserialize file and run inference"
                  << std::endl;
        return -1;
    }
    ...
    for (int j = 0; j < out_dims.nbDims; j++) {
        ...
        std::cout <<"outdims "<<j <<out_dims.d[j]<<std::endl;
    }
